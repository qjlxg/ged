...
def process_file(file_path):
    try:
        ...
        # --- 增强数据清洗：拦截净值异常跳变为1.0的情况 ---
        if len(df) < 30: return None
        curr_p = df['收盘'].iloc[-1]
        prev_p = df['收盘'].iloc[-2]
        # 如果最新价是1.0，但昨天价格远高于1.1，判定为数据源错误（缺失填充），直接剔除
        ...
        try:
            ...
            for _, sig in h_df.iterrows():
                ...
                if not idx_list.empty:
                    ...
                    # 统计端清洗：如果最新价还是异常的1.0，跳过不计入总盈亏计算
